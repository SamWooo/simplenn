...
def addNoise(gaussianModel):
    ...
    for idx, t in enumerate([0, 50, 100, 200, 499]):
        x_noisy = gaussianModel.q_sample(x_start, t=torch.tensor([t]))
...
def trainModel(epochs, device, gaussianModel, optimizer, train_loader):
    ...
    for epoch in range(epochs):
        ...
        for step, (images, labels) in enumerate(train_loader):
            ...
            loss = gaussianModel.train_losses(model, images, t)
...
batch_size = 128
dataLoader = getTrainDataLoader(batch_size)

def showAddNoiseToImage():
    addNoise(gaussian_diffusion)
...
def ddimSampleImageByModel():
    model.load_state_dict(torch.load('./pth/ddpm_cifar.pth'))
    ddimSampleModel(model, gaussian_diffusion)

#showAddNoiseToImage()
#trainDdpmModel()
#sampleImageByModel()
#showDenoiseProcess()
ddimSampleImageByModel()
